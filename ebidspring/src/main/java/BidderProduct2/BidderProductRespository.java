...
class BidderProductDao {
	...
	public BidderProductDao(DataSource dataSource) {
		this.dataSource = dataSource;
	}

	public void updateBidderProductPrice(Long bidderId, Long productId, Long price) {
		String sql = "UPDATE bidder_product1 SET bid_amt = ? WHERE bid = ? AND pid = ?";
		try (Connection conn = dataSource.getConnection(); PreparedStatement ps = conn.prepareStatement(sql)) {
			...
			if (count == 1) {
				...
				System.out.println("No Bidder and product id matching available");
			}
